...
router.post('/view', (req, res) => {
    ...
    if (req.body.tipe_user==1 || req.body.tipe_user==8) {
        ...
        instansi = `AND opd_kkr.id_instansi = '`+req.body.unit_kerja+`'`
    }
    ...
    FROM trista_monev.kk_reviu kk_reviu

    LEFT JOIN egov.users users
    ON users.id = kk_reviu.createBy

    LEFT JOIN simpeg.instansi instansi
    ON kk_reviu.unit_kerja_id = instansi.id

    LEFT JOIN simpeg.unit_kerja unit_kerja
    ON kk_reviu.unit_kerja_id = unit_kerja.id

    LEFT JOIN simpeg.biodata biodata
    ON biodata.nip = users.nama_nip

    LEFT JOIN trista_monev.opd_kkr opd_kkr
    ON kk_reviu.id = opd_kkr.id_kkr

    WHERE 
    kk_reviu.uraian LIKE '%`+cari+`%'

    `+instansi+`
    
    ORDER BY kk_reviu.createAt DESC
...
router.post('/addData',upload.single("file"), (req,res)=>{
    ...
    var id = uniqid();
     ...
     let insert = `INSERT INTO kk_reviu (id, unit_kerja_id, tahun, uraian, keterangan, file, file_type, createAt, createBy)
    VALUES (
        '`+id+`',
        '`+pb.unit_kerja_id+`',
        '`+pb.tahun+`',
...
function masukanOpd(params, id) {
    ...
    for(var i in data){
        ...
        let insert = `
        INSERT INTO opd_kkr (id, id_kkr, id_instansi)
        VALUES (
        '`+uniqid()+`' ,
        '`+id+`',
        '`+data[i].id+`' 
...
router.post('/removeData', (req, res)=> {
    ...
    var query = `
        DELETE FROM kk_reviu WHERE id = '`+req.body.id+`';
    `;

    var opdkkr = `
    DELETE FROM opd_kkr WHERE id_kkr = '`+req.body.id+`';
`;
