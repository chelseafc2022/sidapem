...
router.post('/view', (req, res) => {
    ...
    master_pj.uraian as jabatan_pj,
    ...
    ON kertas_kerja.jabatan_pj = master_pj.id
    ...
    ON users.id = kertas_kerja.createBy
        ...
        WHERE kertas_kerja.jenis_adm = '1' AND 
        `+instansi+`
...
router.post('/addDatax',upload.single("file"), (req,res)=>{
    ...
    if (file==undefined) {
        ...
        let insert = `INSERT INTO tanggapan (id, id_bku, unit_kerja_id, jenis_adm, id_kka, uraian, status, tanggapan, createAt, createBy)
       VALUES (
           '`+uniqid()+`',
           '`+data.id_bku+`',
           '`+data.unit_kerja_id+`',
...
router.post('/editData', (req,res)=>{
    ...
    tglsampai = '`+data.tglsampai+`',
    nomor = '`+data.nomor+`',
    nilai = '`+data.nilai+`',
    penyedia = '`+data.penyedia+`',
    realisasi_fisik = '`+data.realisasi_fisik+`',
    selisih_realisasi = '`+data.selisih_realisasi+`',
    realisasi_keuangan = '`+data.realisasi_keuangan+`',
