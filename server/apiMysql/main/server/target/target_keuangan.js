...
async function getOne(req, res){
    return new Promise((resolve, reject) => {
        let view = ` 
            ...
            WHERE 
            (target_keuangan.rincian_id = `+req.body.rincian_id+` AND target_keuangan.thn = `+req.body.thn+`) AND
            target_keuangan.status = `+req.body.status+`
        `;
...
async function addData(req, res){
    return new Promise((resolve, reject) => {
        let insert = `
            INSERT INTO target_keuangan (rincian_id, jan, feb, mar, apr, mei, jun, jul, agu, sep, okt, nov, des, thn, createBy, createAt)
            VALUES (
                '`+req.body.rincian_id+`',
                `+req.body.jan+`,
                `+req.body.feb+`,
...
async function editData(req, res){
    return new Promise((resolve, reject) => {
        ...
        var query = `
            ...
            WHERE (target_keuangan.rincian_id = `+req.body.rincian_id+` AND target_keuangan.thn = `+req.body.thn+`) AND
            target_keuangan.status = `+req.body.status+`
        `;
