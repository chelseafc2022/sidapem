...
router.post('/list', (req, res) => {
    ...
    if (req.body.urusan_kode == null || req.body.urusan_kode == undefined || req.body.urusan_kode == "") {
        ...
        filterByUrusan = `WHERE master_kegiatan.urusan_kode = '`+req.body.urusan_kode+`'`
    }
...
router.post('/addData', (req, res) => {
    ...
    let insert = `
        ...
        VALUES (
            ...
            '`+req.body.kode+`',
            '`+req.body.program_kode+'.'+req.body.kode+`',
            '`+req.body.uraian+`',
            '`+req.body.keterangan+`',
            '`+req.user._id+`',
            NOW()
...
router.post('/editData', (req,res)=>{
    var query = `
        UPDATE master_kegiatan SET
        program_kode = '`+req.body.program_kode+`',
        kode = '`+req.body.kode+`',
        kode_full = '`+req.body.program_kode+'.'+req.body.kode+`',
...
router.post('/removeData', (req, res)=> {
    var query = `
        DELETE FROM master_kegiatan WHERE id = '`+req.body.id+`';
    `;
    db.query(query, (err, row)=>{
        if(err){
            res.send(err);
