<template>
  <q-list >
    ...
    <q-item-label class="main1" header><span style="color:white">{{nama}}</span></q-item-label>
    ...
    <template v-for="(data, index) in $store.state.list_menu">
        ...
        <q-item v-if="!checkLength(data.subItem)" clickable :to="data.route" exact :key="data.index">
          <q-item-section top avatar>
            ...
            <q-avatar class="iconSize" :color="data.color" text-color="white" :icon="data.icon" />
          </q-item-section>
          <q-item-section>
        ...
        <q-expansion-item  v-if="checkLength(data.subItem)" :key="data.index">
          <template v-slot:header>
            <q-item-section avatar>
              <q-avatar class="iconSize" :icon="data.icon" :color="data.color" text-color="white" />
            </q-item-section>
...
<!-- 
    ...
    <q-item clickable @click="logout" exact>
      <q-item-section top avatar>
        <q-avatar class="iconSize" color="red" text-color="white" icon="lock" />
      </q-item-section>
      <q-item-section>
...
export default {
  ...
  methods: {
    ...
    async loadMenu(){
      ...
      var dataMenu =await this.FETCHING.postMasterMenuGetSideBar(profileku.sidapem)
